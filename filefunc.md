...
| `.env` | 本地环境变量（API Key、超时、代理、历史记录配置），不应提交。 |
| `.gitignore` | Git 忽略规则（含 `data/history.db`）。 |
...
| 路径 | 作用 |
|---|---|
| `server/services/paths.py` | 统一路径常量（项目根、`data/kbs`、`manifest`、`history.db`）。 |
| `server/services/manifest.py` | KB manifest 读写与 upsert。 |
| `server/services/kb_store.py` | kb_id 生成、上传落盘、KB 列表、索引路径管理。 |
| `server/services/rag_service.py` | 面向 API 的 RAG 封装（建库、拒答、citations score 输出）。 |
| `server/services/history_store.py` | 历史记录存储与查询（SQLite，TextLab/RAGLab）。 |
...
| `web/src/lib/api.js` | 统一 fetch 封装，默认 `http://localhost:8000`。 |
...
| `data/kbs/` | Web KB 数据目录（上传 raw、index、manifest）。 |
| `data/history.db` | Web 历史记录 SQLite（TextLab/RAGLab）。 |
...
- `data/history.db`：历史记录存储（可通过 `HISTORY_DB_PATH` 指定）
- `web/dist/`：前端构建产物

## 8. 维护建议
- 新增或修改 API 时同步更新 `README.md` 与 `operate.md`。
- 修改 RAG 切分或索引策略后必须重建索引。
- 需要清空历史记录时，删除 `data/history.db` 或设置新的 `HISTORY_DB_PATH`。
